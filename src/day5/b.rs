...
use std::{collections::BinaryHeap, io::{stdin, stdout, BufWriter, Write}};
...
fn main() {
  ...
  let mut ids_heap = BinaryHeap::new();
  ...
  let mut prev = -ids_heap.pop().unwrap();
  loop {
    let next = -ids_heap.pop().unwrap();
    if next != prev + 1 {
      writeln!(out, "{}", prev + 1).unwrap();
      break;
    } else {
      prev = next;
    }
